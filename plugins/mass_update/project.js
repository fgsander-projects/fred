$j(function() {
	...
	var fieldChangeHandler = function() {
		var form = AppGiniPlugin.commandForm,
			t = project.getTable(AppGiniPlugin.tableIndex),
			fieldName = $j('#field').val(),
			field = project.getField(t.name, fieldName),
		...
		/* for option list fields, populate value drop-down with options */
		} else if (typeof(field.CSValueList) == 'string') {
			/* show all values */
			var csvOptions = [optionAllowUserToSpecify],
				rawCSV = field.CSValueList.split(';;');

			for(var i = 0; i < rawCSV.length; i++)
				csvOptions.push({ id: rawCSV[i], text: rawCSV[i] });

			form.get('value').setOptions(csvOptions);
	...
	var valueChangeHandler = function() {
		...
		AppGiniPlugin.commandForm.get('fixed-value').display(val == 'fixedValue');
		if(val == 'fixedValue') $j('#fixed-value').focus();
	}

	/* populate command editor form with specified command config */
	var loadCommand = function(cmdId) {
		var cmd = getCommand(AppGiniPlugin.tableName, cmdId);
		...
		$j('#fixed-value').val(cmd.fixedValue);
	...
	var appendCommandCounterToTablesList = function() {
		...
		$j('#tables-list a').each(function() {
			var index = $j(this).data('table-index');
