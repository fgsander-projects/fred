...
class custom_category extends WP_Widget {
	public function __construct() {
		$widget_ops = array('description' => esc_html__( "Display latest and selected post categories of different post type.", 'vestige') );
		parent::__construct('name', esc_html__( 'Custom Categories','vestige'), $widget_ops);
	}
	...
	?>
        ...
        <p>
			<input class="checkbox" type="checkbox" <?php echo esc_attr($show); ?> id="<?php echo esc_attr($this->get_field_id('show')); ?>" name="<?php echo esc_attr($this->get_field_name('show')); ?>" /> <label for="<?php echo esc_attr($this->get_field_id('show')); ?>"><?php esc_html__('Display as dropdown', 'vestige'); ?></label></p>
	<?php
	}
         // update widget
  ...
  <button type="button" class="btn btn-default dropdown-toggle" style="font-family:sans-serif; color:#777" data-toggle="dropdown" aria-expanded="false">
    '.$type.' '.esc_html__( 'Categories','vestige').' <span class="caret"></span>
