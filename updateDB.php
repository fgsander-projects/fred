<?php
	...
	if($thisMD5==$prevMD5){
		...
		// set up tables
		setupTable('item', "create table if not exists `item` (   `id` INT unsigned not null auto_increment , primary key (`id`), `identificacao` VARCHAR(80) not null , unique `identificacao_unique` (`identificacao`), `numero_serie` INT not null , `colecao` INT unsigned not null , `colecao_codigo` INT unsigned null , `grupo` INT unsigned not null , `grupo_codigo` INT unsigned null , `serie` INT unsigned not null , `serie_codigo` INT unsigned null , `subserie` INT unsigned null , `titulo` TEXT null , `titulo_atribuido` VARCHAR(40) null , `descricao` TEXT null , `tipologia` INT unsigned not null , `date` DATE null , `data_livre` VARCHAR(120) null , `data_atribuida` VARCHAR(40) null , `autoria` TEXT null , `quantidade` VARCHAR(40) null , `idioma` BLOB null , `local_producao` TEXT null , `local_publicacao_veiculo` INT unsigned null , `local_publicacao` TEXT null , `tipo_publicacao` INT unsigned null , `genero` BLOB null , `forma` TEXT null , `formato` BLOB null , `escritura` TEXT null , `suporte` BLOB null , `dimensao` VARCHAR(120) null , `estado_conservacao` TEXT null , `observacoes` TEXT null , `documentos_relacionados` BLOB null , `numero_caixa` INT unsigned null , `nome_caixa` INT unsigned null , `numero_pasta` INT unsigned null , `nome_pasta` INT unsigned null , `upload` VARCHAR(40) null , `usuario_cadastro` VARCHAR(80) null , `usuario_alteracao` VARCHAR(80) null , `publicar` VARCHAR(40) null , `uploads` TEXT null ) CHARSET utf8", $silent, array( " ALTER TABLE `item` CHANGE `suporte` `suporte` TEXT null "," ALTER TABLE `item` CHANGE `suporte` `suporte` BLOB null "," ALTER TABLE `item` CHANGE `formato` `formato` BLOB null "));
		setupIndexes('item', array('colecao','grupo','serie','subserie','tipologia','local_publicacao_veiculo','tipo_publicacao','numero_caixa','nome_caixa','numero_pasta','nome_pasta'));
		...
		setupTable('items_salvos', "create table if not exists `items_salvos` (   `id` INT unsigned not null auto_increment , primary key (`id`), `memberID` VARCHAR(40) null , `tableName` VARCHAR(40) null , `pkValue` VARCHAR(40) null , `groupID` VARCHAR(40) null , `dateAdded` DATE null , `text` TEXT null ) CHARSET utf8", $silent);
