...
class vestige_selected_post extends WP_Widget
{
	// constructor
	public function __construct()
	{
		$widget_ops = array('description' => esc_html__('Display latest and selected post of different post type.', 'vestige-core'));
		parent::__construct(false, $name = esc_html__('Selected Post', 'vestige-core'), $widget_ops);
	}
	...
	<p>
		<label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php _e('Number of posts to show', 'vestige-core'); ?></label>
	...
	if ($posts_listing->have_posts()) {
		...
		while ($posts_listing->have_posts()) {
			...
			echo '	  </a>
						...
						<span class="meta-data">' . esc_html__('Posted on ', 'vestige-core') . esc_attr(get_the_date()) . '</span>
						</div>
					</li>';
		}
		echo '</ul>';
	} else {
		echo __('No ', 'vestige-core') . $type . esc_html__(' Found', 'vestige-core');
	}
	wp_reset_postdata();
...
add_action('widgets_init', function () {
	register_widget('vestige_selected_post');
});
?>