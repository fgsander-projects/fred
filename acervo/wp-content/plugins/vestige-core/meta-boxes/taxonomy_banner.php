...
function vestige_image_category_tax_custom_fields($tag) {
       ...
       <table class="form-table">
           <tbody><tr class="form-field form-required">
                   <th scope="row"><label for="image"><?php esc_html_e('Term Banner Image', 'vestige') ?></label></th>
                   <td><?php
                       echo '<div><img id ="banner_image_preview" src ="' . $term_meta . '" width ="150px" height ="150px"/></div>';
                       echo '<input id="upload_term_button" type="button" class="button button-primary" value="'.esc_html__('Upload Image', 'vestige').'" /> ';
                      if(isset($_REQUEST['tag_ID'])){
                       echo '<input id="upload_term_button_remove" type="button" class="button button-primary" value="'.esc_html__('Remove Image', 'vestige').'" />';
                      }
                       ?>
                   <p class="description"><?php esc_html_e('Upload an image for the taxonomy .', 'vestige'); ?></p></td>
