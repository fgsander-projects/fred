...
if (!class_exists('ReduxFramework_extension_wbc_importer')) {
    class ReduxFramework_extension_wbc_importer
    {
        ...
        public function __construct($parent)
        {
            ...
            include_once VESTIGE_CORE__PLUGIN_PATH . 'imi-admin/theme-options/extensions/wbc_importer/imi-wbc-configs.php';
        }
        ...
        public function demoFiles()
        {
            ...
            $dir_array = array(
                'classic' => array(
                    ...
                    'preview_url' => 'https://demo.imithemes.com/vestige-wp/',
                    ...
                    'files' => array_merge($fix_options, array(
                        ...
                        'highlight-showcase7.zip' => array(
                            'name' => 'highlight-showcase7.zip',
                            'type' => 'f',
                        ),
                        'vestige.zip' => array(
                            'name' => 'vestige.zip',
        ...
        public function create_demo_dir()
        {
            ...
            if (imi_check_url('https://data.imithemes.com/demo-data/vestige/' . $target_demo . '/' . $target_demo . '.zip')) {
                $value = 'https://data.imithemes.com/demo-data/vestige/' . $target_demo . '/' . $target_demo . '.zip';
            } else {
                $value = 'https://preview.imithemes.com/demo-data/vestige/' . $target_demo . '/' . $target_demo . '.zip';
            }
        ...
        public function ajax_importer()
        {
            ...
            if (isset($_REQUEST['type']) && $_REQUEST['type'] == "import-demo-content" && array_key_exists($_REQUEST['demo_import_id'], $this->wbc_import_files)) {
                ...
                if (!isset($import_parts['imported']) || true === $reimporting) {
                    ...
                    echo esc_html__("Demo Already Imported", 'vestige');
                }
        ...
        function add_importer_section()
        {
            ...
            $wbc_importer_label = trim(esc_html(apply_filters('wbc_importer_label', __('Demo Importer', 'vestige'))));

            $wbc_importer_label = (!empty($wbc_importer_label)) ? $wbc_importer_label : __('Demo Importer', 'vestige');

            $this->parent->sections[] = array(
                'id'     => 'wbc_importer_section',
                'title'  => $wbc_importer_label,
                'desc'   => '<p class="description">' . apply_filters('wbc_importer_description', esc_html__('Works best to import on a new install of WordPress', 'vestige')) . '</p>',
