...
if ( ! class_exists( 'Envato_Market_Admin' ) && class_exists( 'Envato_Market' ) ) :
	...
	class Envato_Market_Admin {
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'vestige' ), '1.0.0' );
		}
		...
		public function maybe_delete_transients() {
			if ( isset( $_POST[ envato_market()->get_option_name() ] ) ) {
				...
				if ( isset( $_POST['_wpnonce'] ) && ! wp_verify_nonce( $_POST['_wpnonce'], envato_market()->get_slug() . '-options' ) ) {
					wp_die( __( 'You do not have sufficient permissions to delete transients.', 'vestige' ) );
				}
		...
		public function add_menu_page() {
			$page = add_menu_page(
				__( 'Envato Market', 'vestige' ), __( 'Envato Market', 'vestige' ), 'manage_options', envato_market()->get_slug(), array(
					$this,
		...
		public function admin_enqueue_script() {
			...
			$exports = array(
				...
				'i18n'   => array(
					'save'   => __( 'Save', 'vestige' ),
					'remove' => __( 'Remove', 'vestige' ),
					'cancel' => __( 'Cancel', 'vestige' ),
					'error'  => __( 'An unknown error occurred. Try again.', 'vestige' ),
				),
			);
		...
		public function render_templates() {
			...
			<script type="text/html" id="tmpl-envato-market-auth-check-button">
				<a href="<?php echo esc_url( add_query_arg( array( 'authorization' => 'check' ), envato_market()->get_page_url() ) ); ?>" class="button button-secondary auth-check-button" style="margin:0 5px"><?php esc_html_e( 'Test API Connection', 'vestige' ); ?></a>
			</script>

			<script type="text/html" id="tmpl-envato-market-item">
				<li data-id="{{ data.id }}">
					<span class="item-name"><?php esc_html_e( 'ID', 'vestige' ); ?>: {{ data.id }} - {{ data.name }}</span>
					<button class="item-delete dashicons dashicons-dismiss">
						<span class="screen-reader-text"><?php esc_html_e( 'Delete', 'vestige' ); ?></span>
					</button>
			...
			<script type="text/html" id="tmpl-envato-market-dialog-remove">
				<div id="envato-market-dialog-remove" title="<?php esc_html_e( 'Remove Item', 'vestige' ); ?>">
					<p><?php esc_html_e( 'You are about to remove the connection between the Envato Market API and this item. You cannot undo this action.', 'vestige' ); ?></p>
				</div>
			</script>

			<script type="text/html" id="tmpl-envato-market-dialog-form">
				<div id="envato-market-dialog-form" title="<?php esc_html_e( 'Add Item', 'vestige' ); ?>">
					<form>
						<fieldset>
							<label for="token"><?php esc_html_e( 'Token', 'vestige' ); ?></label>
							<input type="text" name="token" class="widefat" value=""/>
							<p class="description"><?php esc_html_e( 'Enter the Envato API Personal Token.', 'vestige' ); ?></p>
							<label for="id"><?php esc_html_e( 'Item ID', 'vestige' ); ?></label>
							<input type="text" name="id" class="widefat" value=""/>
							<p class="description"><?php esc_html_e( 'Enter the Envato Item ID.', 'vestige' ); ?></p>
						...
						<div class="envato-card-top">
							...
							<div class="column-name">
								<h4>
									...
									<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'vestige' ); ?>"><?php esc_html_e( 'Version', 'vestige' ); ?> {{ data.version }}</span>
								</h4>
							</div>
							<div class="column-description">
								...
								<p class="author">
									<cite><?php esc_html_e( 'By', 'vestige' ); ?> {{ data.author }}</cite>
								</p>
							</div>
						</div>
						<div class="envato-card-bottom">
							<div class="column-actions">
								<a href="{{{ data.install }}}" class="button button-primary">
									<span aria-hidden="true"><?php esc_html_e( 'Install', 'vestige' ); ?></span>
									<span class="screen-reader-text"><?php esc_html_e( 'Install', 'vestige' ); ?> {{ data.name }}</span>
								</a>
							</div>
						</div>
					</div>
		...
		public function register_settings() {
			...
			add_settings_section(
				...
				__( 'Activate Envato API Connection', 'vestige' ),
			...
			add_settings_section(
				...
				__( 'Single Item Tokens (Advanced)', 'vestige' ),
			...
			add_settings_field(
				'items',
				__( 'Envato Market Items', 'vestige' ),
		...
		public function install_plugin( $plugin ) {
			if ( ! current_user_can( 'install_plugins' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . __( 'Installing Plugin...', 'vestige' ) . '</h1>
					<p>' . __( 'You do not have sufficient permissions to install plugins on this site.', 'vestige' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . __( 'Return to Plugin Installer', 'vestige' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . __( 'Installing Plugin...', 'vestige' ) . '</h1>
					<p>' . __( 'An error occurred, please check that the item ID is correct.', 'vestige' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . __( 'Return to Plugin Installer', 'vestige' ) . '</a>
				</div>';
				wp_die( $msg );
			}

			$title              = sprintf( __( 'Installing Plugin: %s', 'vestige' ), esc_html( $api->name . ' ' . $api->version ) );
		...
		public function install_theme( $theme ) {
			if ( ! current_user_can( 'install_themes' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . __( 'Installing Theme...', 'vestige' ) . '</h1>
					<p>' . __( 'You do not have sufficient permissions to install themes on this site.', 'vestige' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . __( 'Return to Theme Installer', 'vestige' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . __( 'Installing Theme...', 'vestige' ) . '</h1>
					<p>' . __( 'An error occurred, please check that the item ID is correct.', 'vestige' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . __( 'Return to Plugin Installer', 'vestige' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			$title              = sprintf( __( 'Installing Theme: %s', 'vestige' ), esc_html( $api->name . ' ' . $api->version ) );
		...
		public function ajax_add_item() {
			if ( ! check_ajax_referer( self::AJAX_ACTION, 'nonce', false ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'The Token is missing.', 'vestige' ) ) );
			} elseif ( empty( $_POST['id'] ) ) {
				wp_send_json_error( array( 'message' => __( 'The Item ID is missing.', 'vestige' ) ) );
			}
			...
			if ( false === $request ) {
				wp_send_json_error( array( 'message' => __( 'The Token or Item ID is incorrect.', 'vestige' ) ) );
			}

			if ( false === envato_market()->api()->download( $_POST['id'], $args ) ) {
				wp_send_json_error( array( 'message' => __( 'The item cannot be downloaded.', 'vestige' ) ) );
			}
			...
			wp_send_json_error( array( 'message' => __( 'An unknown error occurred.', 'vestige' ) ) );
		}
