...
{
    add_meta_box('event_schedule',esc_html__('Event Tickets Type','vestige'),'imic_event_feilds_output','event','normal','core');
}
/**
...
<div id="field_group">
    ...
    if ( !empty( $tickets_type) ) 
    {
        ...
        <?php if($row_head==1)
				{ 
					echo '<label class="heading1">'.esc_html__('Ticket Title', 'vestige').'</label>';
					echo '<label class="heading2">'.esc_html__('Total Tickets', 'vestige').'</label>';
					echo '<label class="heading3">'.esc_html__('Booked Tickets', 'vestige').'</label>';
					echo '<label class="heading4">'.esc_html__('Ticket Cost', 'vestige').'</label>';
					echo '<br/>';
				}
					?>
        <div class="field_row">
...
function add_admin_scripts_event( $hook ) {
    ...
    if ( $hook == 'post-new.php' || $hook == 'post.php' ) {
        if ( 'event' === $post->post_type ) {     
            wp_enqueue_script(  'event_clone_tickets_js', IMIC_THEME_PATH.'/assets/js/clone_fields.js' );
			wp_enqueue_style(  'event_clone_tickets_style', IMIC_THEME_PATH.'/assets/css/clone_fields.css' );
        }
    }
}
