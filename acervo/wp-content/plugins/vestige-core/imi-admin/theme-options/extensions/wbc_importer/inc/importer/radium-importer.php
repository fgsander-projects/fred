...
if ( !class_exists( 'Radium_Theme_Importer' ) ) {
  ...
  class Radium_Theme_Importer {
    ...
    public function after_wp_importer() {
      ...
      do_action( 'wbc_importer_after_content_import', $this->active_import, $this->demo_files_path, $this->import_sliders, $this->import_theme_opts );
    ...
    public function process_imports() {
      ...
      if ( $this->import_widgets && !empty( $this->widgets ) && is_file( $this->widgets ) ) {
    ...
    public function set_demo_data( $file ) {
      ...
      if ( $importer_error ) {
        ...
        if ( !is_file( $file ) ) {
          ...
          $wp_import->import( $file, $this->import_contents );
    ...
    public function set_demo_theme_options( $file ) {
      ...
      if ( ! file_exists( $file ) ) {
        wp_die(
          __( 'Theme options Import file could not be found. Please try again.', 'vestige' ),
          '',
    ...
    function process_widget_import_file( $file ) {
      ...
      if ( ! file_exists( $file ) ) {
        wp_die(
          __( 'Widget Import file could not be found. Please try again.', 'vestige' ),
          '',
          array( 'back_link' => true )
        );
      }
    ...
    public function import_widgets( $data ) {
      ...
      foreach ( $data as $sidebar_id => $widgets ) {
        ...
        if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
          ...
          $sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'vestige' );
        }
        ...
        foreach ( $widgets as $widget_instance_id => $widget ) {
          ...
          if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
            ...
            $widget_message = __( 'Site does not support widget', 'vestige' ); // explain why widget not imported
          }
          ...
          if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
            ...
            foreach ( $single_widget_instances as $check_id => $check_widget ) {
              ...
              if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
                ...
                $widget_message = __( 'Widget already exists', 'vestige' ); // explain why widget not imported
          ...
          if ( ! $fail ) {
            ...
            if ( $sidebar_available ) {
              ...
              $widget_message = __( 'Imported', 'vestige' );
            } else {
              $widget_message_type = 'warning';
              $widget_message = __( 'Imported to Inactive', 'vestige' );
            }
          ...
          $results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'vestige' ); // show "No Title" if widget instance is untitled
